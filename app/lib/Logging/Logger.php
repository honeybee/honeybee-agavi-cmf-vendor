...
class Logger extends AgaviLogger
{
    ...
    public static function getLevelName($level)
    {
        ...
        if (!isset(static::$levels[$level])) {
            throw new InvalidArgumentException(
                sprintf(
                    'Log level "%s" is undefined, use one of: %s',
                    $level,
    ...
    /**
     * @return \Honeygavi\Logging\Psr3Logger instance that is compatible to the PSR-3 standard
     */
    ...
    public function setAppender($name, AgaviLoggerAppender $appender)
    {
        ...
        throw new AgaviLoggingException(
            sprintf('An appender with the name "%s" is already registered.', $name)
        );
    }
}
