...
use AgaviContext;
use AgaviLogger;
use AgaviLoggerManager;
use AgaviValidationManager;
use AgaviWebRequest;
use DateTime;
use Exception;
...
class LoggerManager extends AgaviLoggerManager implements ILogger//, \Psr\Log\LoggerAwareInterface
{
     ...
     *              NULL should fallback to the default logger
     * @param int $log_level log level to use for logger message creation
     * @param string $scope string or object implementing __toString() for scope of log message
     *              (e.g. callee class name or sub channel name)
    ...
    public function logTo(
        $logger_name = null,
        $log_level = AgaviLogger::INFO,
        $scope = self::DEFAULT_MESSAGE_SCOPE,
        $log_message_parts = ""
    ) {
        ...
        /* @var $logger Honeygavi\Logging\Logger */
        $logger = $this->getLogger($logger_name);

        if (!$logger) {
            throw new InvalidArgumentException(
                "Can't find logger with name '" . $logger_name . "'." .
                "Please specify another name or define the logger in the logging.xml file."
            );
        }
     ...
     * @throws InvalidArgumentException when there's no logger configured for the given logger name
     */
    public static function logLoggerAndLevel(
        $logger_name = null,
        $log_level = AgaviLogger::INFO,
        $scope = self::DEFAULT_MESSAGE_SCOPE,
        $log_message_parts = ""
        ...
        /* @var $logger_manager Honeygavi\Logging\LoggerManager */
     ...
     * Honeybee Module and Entity instances.
     *
    ...
    public static function getAsString($log_message_part)
    {
        if ($log_message_part instanceof Exception) {
            ...
            return (string) sprintf('resource(type=%s)', get_resource_type($log_message_part));
        } elseif (true === $log_message_part) {
            return 'true';
        } elseif (false === $log_message_part) {
            return 'false';
        } elseif (null === $log_message_part) {
            return 'null';
        }
    ...
    public static function getExceptionAsString(Exception $exception)
    {
        ...
        $extra['Timestamp'] = DateTime::createFromFormat(
            'U.u',
        ...
        $extra['Memory Usage'] = StringToolkit::formatBytes(memory_get_usage(true));
        $extra['Memory Peak Usage'] = StringToolkit::formatBytes(memory_get_peak_usage(true));
        ...
        if (null !== ($request = $agavi_context->getRequest())) {
            ...
            if (!$request->isLocked()) {
                $extra['Parameter Names (Request)'] = implode(
                    ', ',
                    array_keys($request->getRequestData()->getParameters())
                );
            }
        }
     ...
     * Module or Entity. The given object should implement a `__toString()`
    ...
    {
        if ($obj instanceof AggregateRootType) {
            return 'Aggregate (Name=' . $obj->getName() . ')';
        } elseif ($obj instanceof AggregateRoot) {
            return 'AggregateRoot (Identifier=' . $obj->getIdentifier() . ')';
    ...
    public static function getExtraInformation()
    {
        ...
        if (!empty($first_container)) {
            ...
            $first_module_action_and_view = $first_container->getModuleName() .
                '/' . $first_container->getActionName() .
                ' => ' . $first_container->getViewModuleName() .
                '/' . $first_container->getViewName();
        }
        ...
        $extra = array(
            ...
            'system' => php_uname(),
            'pid' => getmypid(),
            'memory_usage' => StringToolkit::formatBytes(memory_get_usage(true)),
            'memory_peak_usage' => StringToolkit::formatBytes(memory_get_peak_usage(true))
        );
