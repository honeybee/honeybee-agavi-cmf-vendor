 ...
 *
 * setup - Honeygavi\Logging\Monolog\MonologSetupInterface implementing class name
...
class MonologLoggerAppender extends AgaviLoggerAppender
{
    ...
    {
        if (!$this->logger) {
            $setup = $this->getParameter('setup', DefaultSetup::CLASS);
            if (class_exists($setup)) {
                $this->logger = $setup::getMonologInstance($this);
            } else {
                throw new AgaviLoggingException('Parameter "setup" specifies a non-existing class: ' . $setup);
            }
        }
    ...
    public function convertAgaviLevelToMonologLevel($log_level_or_severity)
    {
        if (!is_int($log_level_or_severity)) {
            throw new InvalidArgumentException(
                "The given log level is not an integer: " . $log_level_or_severity .
                " - Please use AgaviLogger::DEBUG or similar."
            );
        }
