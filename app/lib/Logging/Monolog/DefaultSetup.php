 ...
 */
class DefaultSetup implements MonologSetupInterface
{
    /**
    ...
    public static function getMonologInstance(AgaviLoggerAppender $appender)
    {
        ...
        $file_path = preg_replace(
            '/[^a-zA-Z0-9-_\.\/]/',
            '',
            $appender->getParameter('destination', $default_file_path)
        );
        $syslog_identifier = $appender->getParameter(
            'syslog_identifier',
            AgaviConfig::get('core.app_name', __METHOD__)
        );
        $syslog_facility = $appender->getParameter('syslog_facility', LOG_USER);
        $formatter_template = $appender->getParameter(
            'formatter_template',
            "%message% [%channel%.%level_name%] %context% %extra%\n"
        );
