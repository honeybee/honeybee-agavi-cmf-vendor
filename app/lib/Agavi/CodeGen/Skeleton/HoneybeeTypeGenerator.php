...
class HoneybeeTypeGenerator extends SkeletonGenerator
{
    /**
    ...
    {
        $required_data_keys = [ 'vendor', 'package', 'type' ];
        ...
        $data['type_prefix'] = StringToolkit::asSnakeCase($type);
    ...
    public function generate()
    {
        ...
        // Now that a new type skeleton has now been created, we wil generate
        // the aggregate root and standard projection files immediately.
        $this->executeTrellis('aggregate_root');
        $this->executeTrellis('standard');

        // Copy Trellis ES mapping file to correction location
        $trellis_config_path = $this->getTargetPath('standard') . 'standard.ini';
        ...
        $mapping_target_path = sprintf(
            '%1$s%2$smigration%2$selasticsearch%2$s%3$s_create_%4$s_type%2$s%5$s',
            $this->data['target_path'],
            DIRECTORY_SEPARATOR,
            $this->data['timestamp'],
            $this->data['type_prefix'],
